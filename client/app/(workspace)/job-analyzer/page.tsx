...
function AnalysisApplicationRequirements({ application }: { application: ExtractedApplication | JobApplication | null }) {
  if (!application?.requirements.length) return null
  return <section className="analysis-application-requirements"><h3>Requirements</h3><ul>{application.requirements.map(requirement => <li key={`${requirement.name}-${requirement.isRequired}`}>{requirement.name}<small>{requirement.isRequired ? 'Required' : 'Optional'}</small></li>)}</ul></section>
}
...
export default function JobAnalyzerPage() {
  ...
  return <>
    <section className="page-heading"><div><p className="eyebrow">ROLE FIT</p><h1>Job match analyzer</h1><p>Compare one job context with your profile and see the skills you can build next.</p></div></section>
    ...
    <section className="analyzer-grid">
      <article className="panel analyzer-input">
        ...
        {applicationsQuery.isPending ? <div className="content-skeleton analyzer-skeleton"><i /><i /><i /></div> : <>
          {applications.length ? <><label htmlFor="application-select">Saved application</label><select id="application-select" value={selectedId ?? ''} onChange={event => selectApplication(event.target.value)}><option value="">Select an application</option>{applications.map(application => <option key={application.id} value={application.id}>{application.position} — {application.company}</option>)}</select>{selected && <div className="selected-role-card"><span className={`status-pill ${selected.status}`}>{selected.status}</span><h2>{selected.position}</h2><p>{selected.company}</p><div className="requirement-preview"><strong>{selected.requirements.length} requirements</strong>{selected.requirements.length ? <ul>{selected.requirements.slice(0, 8).map(requirement => <li key={requirement.id}><span>{requirement.name}</span><small>{requirement.isRequired ? 'Required' : 'Optional'}</small></li>)}</ul> : <p>This application does not have requirements yet.</p>}</div></div>} </> : <p className="analyzer-help">No saved applications yet. You can still paste a job posting below, or <Link className="text-link" href="/applications?new=1">add an application</Link>.</p>}
          {!selectedId && <><label htmlFor="job-posting-text">Job posting text <span className="field-hint">Use this instead of selecting an application</span></label><textarea id="job-posting-text" className="analyzer-job-text" maxLength={30000} value={jobText} onChange={event => typeJobText(event.target.value)} placeholder="Paste text copied from LinkedIn, Indeed, or another job board..." /></>}
      ...
      <article className="panel analyzer-result">
        ...
        {aiAnalysis ? <>
          ...
          <AnalysisApplicationRequirements application={applicationForResult ?? null} />
...
function AnalysisApplicationCard({ application }: { application: ExtractedApplication | JobApplication | null }) {
  ...
  return <section className="analysis-application-card"><div><span>Company</span><strong>{application.company || '—'}</strong></div><div><span>Position</span><strong>{application.position || '—'}</strong></div><div><span>Location</span><strong>{application.location || '—'}</strong></div><div><span>Salary</span><strong>{formatSalary(application.salary)}</strong></div><div><span>Status</span><strong>{'status' in application ? application.status : 'Saved'}</strong></div><div><span>Applied</span><strong>{formatDate(application.appliedAt)}</strong></div><div><span>Interview</span><strong>{formatDate(application.interviewAt)}</strong></div>{application.jobUrl && <a className="text-link" href={application.jobUrl} target="_blank" rel="noreferrer"><Icon name="external" size={13} />Open posting</a>}</section>
}
