...
export function DashboardView() {
  ...
  return <>
    <section className="page-heading dashboard-heading"><div><p className="eyebrow">YOUR CAREER DASHBOARD</p><h1>Welcome back, {firstName} 👋</h1><p>Here&apos;s your career progress at a glance. Keep going.</p></div></section>
    ...
    <section className={dashboardQuery.isPending ? 'metric-grid is-loading' : 'metric-grid'}>
      ...
      <Metric icon="chart" label="Average match" value={`${Math.round(dashboard?.averageMatchScore ?? 0)}%`} detail="Across analyzed applications" tone="teal"/>
      <article className="image-message-card"><Image className="theme-image theme-image-light" src="/images/careeros/mountain-day.png" fill sizes="320px" alt="" unoptimized/><Image className="theme-image theme-image-dark" src="/images/careeros/mountain-night.png" fill sizes="320px" alt="" unoptimized/><div><small>CAREEROS REMINDER</small><strong>Consistent progress creates extraordinary results.</strong></div></article>
    </section>
...
function Metric({icon,label,value,detail,tone}:{icon:IconName;label:string;value:string|number;detail:string;tone:'blue'|'violet'|'amber'|'teal'}) { return <article className="metric-card"><span className={`soft-icon ${tone}`}><Icon name={icon}/></span><div><p>{label}</p><strong>{value}</strong><small>{detail}</small></div></article> }
