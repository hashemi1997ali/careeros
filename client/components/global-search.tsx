...
export function GlobalSearch({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <div className="command-backdrop modal-backdrop" data-open={open ? 'true' : 'false'} aria-hidden={!open} inert={!open} onMouseDown={(e) => e.currentTarget === e.target && onClose()}>
    <section className="command-panel modal-surface" role="dialog" aria-modal="true" aria-label="Search CareerOS">
      <div className="command-input"><Icon name="search"/><input ref={inputRef} value={query} onChange={(e) => { setQuery(e.target.value); setResults(empty); setLoading(e.target.value.trim().length >= 2) }} placeholder="Search applications, skills and projects..."/><kbd>Esc</kbd></div>
